...
func sendOnlineClients(conn net.Conn) {
	...
	conn.Write([]byte("Online clients: " + clientList + "\n"))
}
...
func handleSelectClient(sender, selectedClient string, conn net.Conn) {
	...
	go func() {
		...
		for {
			...
			selectedConn.Write([]byte(sender + ": " + message + "\n"))
		}
	}()
	...
	go func() {
		...
		for {
			...
			conn.Write([]byte(selectedClient + ": " + message + "\n"))
		}
	}()
...
func handleMessage(sender string, message string) {
	...
	recipientConn.Write([]byte(sender + ": " + messageBody + "\n"))
}
